...
class menu:
    def draw(self, screen, width, height, rect_size, font1, font2):
        ...
        sousOptions = ["Chiffrer", "Décrypter"]
        ...
        for i in range(1,4):
            ...
            if (width-rect_size[0])//2 <= mouse[0] <= (width+rect_size[0])//2 and 200*i <= mouse[1] <= 150+200*i:
            
                rectangle1 = pygame.Rect((width-rect_size[0])/2, 200*i, rect_size[0], rect_size[1]/2)
                pygame.draw.rect(screen, "white", rectangle1, width=2, border_radius=15)
                option_rects.append(rectangle1)

                text = sousOptions[0]

                text = font1.render(text, True, "grey")
                text_box = text.get_rect(center= (width/2,237.5+((i-1)*200)))

                screen.blit(text, text_box)

                rectangle2 = pygame.Rect((width-rect_size[0])/2, 200*i+rect_size[1]/2, rect_size[0], rect_size[1]/2)
                pygame.draw.rect(screen, "white", rectangle2, width=2, border_radius=15)
                sous_option_rects.append(rectangle2)

                text = sousOptions[1]

                text = font1.render(text, True, "grey")
                text_box = text.get_rect(center= (width/2,312.5+((i-1)*200)))
    ...
    def afficherMenu(self, screen, width, height, rect_size, font1, font2):
        afficher = True
        while afficher:
            menu.draw(self, screen, width, height, rect_size, font1, font2)